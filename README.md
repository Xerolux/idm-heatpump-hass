...
- `modbus-connection==4.12.2` · `tmodbus[async-serial]==0.6.2` (direct Modbus socket runtime)
- `idm-heatpump-api[web]==2.4.2` (device logic: register metadata, batching, decoding, model detection, write safety)
...
- **Direct local transport**: raw FC03/FC04 reads and FC16 writes use the exact `modbus-connection==4.12.2` and `tmodbus[async-serial]==0.6.2` runtime; version 4.12.2 is the transport library version, not the IDM integration version
- **API boundary**: `idm-heatpump-api[web]==2.4.2` supplies register metadata, batching, encoding/decoding, model detection and write safety. Since that release the API owns its own exception hierarchy and pymodbus is gone: this integration installs no Modbus stack it does not speak
