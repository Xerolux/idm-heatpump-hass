...
<div align="center">
  ...
  <p>
    ...
    <a href="https://xerolux.github.io/idm-heatpump-hass/docs/"><strong>Documentation</strong></a> ·
...
The configuration flow offers **iDM Smart Energy & Comfort** and **Vanilla**.
Smart adds local COP, cycle and short-cycle analysis plus the safe domestic
hot-water boost controls. Vanilla keeps only the core IDM entities. External
PV, battery and room-sensor forwarding remains separately configurable. See
the [Smart Energy & Comfort guide][wiki-smart].
...
| **🔒 Security** | Local plant control; opt-in cloud AI, Modbus TCP, EEPROM protection, EEPROM-sensitive registers |
...
- `modbus-connection==4.12.1` · `tmodbus[async-serial]==0.6.2` (direct Modbus socket runtime)
- `idm-heatpump-api[web]==2.1.2` (device logic: register metadata, batching, decoding, model detection, write safety)
...
- **Direct local transport**: raw FC03/FC04 reads and FC16 writes use the exact `modbus-connection==4.12.1` and `tmodbus[async-serial]==0.6.2` runtime; version 4.12.1 is the transport library version, not the IDM integration version
- **API boundary**: `idm-heatpump-api[web]==2.1.2` supplies register metadata, batching, encoding/decoding, model detection and write safety. Since that release the API owns its own exception hierarchy and pymodbus is gone: this integration installs no Modbus stack it does not speak
...
The **IDM Navigator 2.0 / 10 / Pro** by [IDM EnergieSysteme GmbH](https://www.idm-energiesysteme.de/) is a modular heat pump control system with a Modbus TCP interface for seamless Home Assistant integration.
