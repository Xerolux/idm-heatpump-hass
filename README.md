...
- `modbus-connection==4.8.1` · `tmodbus[async-serial]==0.5.2` (direct Modbus socket runtime)
...
- **Direct local transport**: raw FC03/FC04 reads and FC16 writes use the exact `modbus-connection==4.8.1` and `tmodbus[async-serial]==0.5.2` runtime; version 4.8.1 is the transport library version, not the IDM integration version
