...
- `idm-heatpump-api[web]==2.4.0` (device logic: register metadata, batching, decoding, model detection, write safety)
...
- **API boundary**: `idm-heatpump-api[web]==2.4.0` supplies register metadata, batching, encoding/decoding, model detection and write safety. Since that release the API owns its own exception hierarchy and pymodbus is gone: this integration installs no Modbus stack it does not speak
