...
- `modbus-connection==4.12.2` · `tmodbus[async-serial]==0.6.2` (direkter Modbus-Socket)
- `idm-heatpump-api[web]==2.4.3` (Gerätelogik: Registermetadaten, Batching, Decoding, Modellerkennung, Schreibschutz)
...
Home Assistant
    ...
    │       │       │
    │       │       └── modbus-connection 4.12.2 + tmodbus 0.6.2
    │       │               │
...
- **Direkter lokaler Transport**: FC03-/FC04-Lesezugriffe und FC16-Schreibzugriffe laufen über exakt `modbus-connection==4.12.2` und `tmodbus[async-serial]==0.6.2`; `4.12.2` ist die Version der Verbindungsbibliothek, nicht die IDM-Integrationsversion
- **API-Grenze**: `idm-heatpump-api[web]==2.4.3` liefert Registermetadaten, Batching, Encoding/Decoding, Modellerkennung und Schreibschutz. Seit diesem Release besitzt die API ihre eigene Fehlerhierarchie, und pymodbus entfällt: die Integration installiert keinen Modbus-Stack mehr, den sie nicht spricht
