...
<div align="center">
  ...
  <p>
    ...
    <a href="https://xerolux.github.io/idm-heatpump-hass/docs/#/home"><strong>Dokumentation</strong></a> ·
...
- `modbus-connection==4.8.1` · `tmodbus[async-serial]==0.5.1` (direkter Modbus-Socket)
- `pymodbus>=3.12.1,<4.0` · `idm-heatpump-api[web]==1.0.2` (vorübergehend gepinnte API-Kompatibilitätsabhängigkeiten)
...
- **Direkter lokaler Transport**: FC03-/FC04-Lesezugriffe und FC16-Schreibzugriffe laufen über `modbus-connection==4.8.1` und `tmodbus[async-serial]==0.5.1`; `4.8.1` ist die Version der Verbindungsbibliothek, nicht die IDM-Integrationsversion
- **API-Kompatibilität**: `idm-heatpump-api[web]==1.0.2` bleibt für Register, Batchplanung, Encoding/Decoding, Modellerkennung und Schreibschutz zuständig; `pymodbus>=3.12.1,<4.0` bleibt vorübergehend installiert, weil die API es weiterhin importiert, besitzt aber nicht den direkten Socket
...
- **Sichtbarer Laufzeit-Stack**: Der Diagnose-Sensor `IDM-Heatpump-API-Version` zeigt die installierte API-Version und führt Integrations-, `modbus-connection`-, `tmodbus`- sowie Pymodbus-Kompatibilitätsversion als Attribute; dieselben Angaben stehen im Diagnoseexport und Startlog
