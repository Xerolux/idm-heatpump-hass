...
| Connection library | `modbus-connection==4.12.2` | Transport contract, connection lifecycle or error semantics change |
| Direct socket backend | `tmodbus[async-serial]==0.6.2` | Wire/backend implementation changes |
| Python register/web library | `idm-heatpump-api[web]==2.4.3` | Register schema, encoding/decoding, batching, model detection, write safety or reusable web-client implementation changes |

The manifest lists the tested runtime in this order:
`modbus-connection==4.12.2`, `tmodbus[async-serial]==0.6.2`,
and `idm-heatpump-api[web]==2.4.3`. The first two packages own the direct
...
pymodbus. `4.12.2` is the version of `modbus-connection`, not an IDM integration
