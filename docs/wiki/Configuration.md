...
| Python register/web library | `idm-heatpump-api[web]==0.9.0` | Register schema, encoding/decoding, Modbus client or reusable web-client implementation changes |

Every integration release pins the exact API version it was tested with. The
integration pins the tested `idm-heatpump-api[web]==0.9.0`, which adds
