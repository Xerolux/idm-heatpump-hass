...
| Navigator 1.7 | expected | n1.x firmware | Read-only 1.x sensor map; PV supplement (74/76/78/82 writes, 4122 read) when address 74 responds | API register model, contract tests | Separate protocol family; automatic detection via Illegal-Data-Address signature, manual model override available. Disable the local web supplement; no 1.x web interface is known. |
