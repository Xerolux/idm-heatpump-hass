...
| Navigator 1.7 | community-tested | n1.x firmware | 1.x sensor map plus the official RW holding block (system/heating-circuit modes, room and flow setpoints, heating curves, limits, bivalence points, solar mode, DHW setpoint); PV supplement (74/76/78/82 writes, 4122 read) when address 74 responds | API register model, contract tests, FHEM capture against a real 1.7 (issue #319) | Separate protocol family; automatic detection via Illegal-Data-Address signature, manual model override available. Holding writes are EEPROM-limited (300 000 cycles per register). Disable the local web supplement; no 1.x web interface is known. |
