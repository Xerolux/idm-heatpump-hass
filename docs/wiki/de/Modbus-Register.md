...
Die Integration fragt keine feste Liste ab. `idm-heatpump-api` erstellt eine
Registerkarte aus der erkannten Navigator-Familie, den aktiven Heizkreisen,
Zonenmodulen/Räumen sowie den optionalen Solar-, ISC-, PV- und
Kaskadenfunktionen. So bleiben nicht unterstützte Funktionsblöcke aus den
normalen Abfragen heraus.

Das maßgebliche, maschinenlesbare Register-Schema wird von
[`idm-heatpump-api`](https://github.com/Xerolux/idm-heatpump-api) erzeugt. Der
Schema-Snapshot enthält zu jedem Register Adresse, Datentyp, Zugriffsflags,
Schreibklasse, Quelle, Quellversion, unterstützte Modelle, Sentinel-Werte und
ein optionales Verifikationslabel.

→ [Vollständigen Registerkatalog öffnen](#vollstandiger-registerkatalog)
...
| System | 1000-1199 | Systemparameter, Temperaturen, Drücke |
| Kaskade/Bivalenz | 1200-1349 | Mehrere Wärmepumpen, Heizstab |
| Heizkreise A-G | 1350-1699 | Einzelne Heizkreise |
| GLT/Energie | 1700-1799 | Fernwartung, Energiemessung |
| Zonen 1-10 | 2000-2999 | Zonenmodule mit Raumregelung |

## Datentypen

| Typ | Beschreibung |
|------|-------------|
| **FLOAT** | IEEE-754-Gleitkommazahl (2 Modbus-Wörter, niederwertiges Wort zuerst) |
| **UCHAR** | 8-Bit-Wert ohne Vorzeichen in einem Modbus-Wort |
| **INT8** | 8-Bit-Wert mit Vorzeichen |
| **UINT16** | 16-Bit-Wert ohne Vorzeichen |
| **INT16** | 16-Bit-Wert mit Vorzeichen |
| **BOOL** | Boolescher Wert 0/1 |
| **BITFLAG** | Bitfeld, dekodiert zu aktiven Flags |

> **Wichtig:** Beim Schreiben von Ganzzahlwerten (`UCHAR`, `INT8`, `INT16`, `UINT16`) wendet die Integration automatisch den im Code hinterlegten `multiplier` an und rundet den Wert entsprechend.
...
| Standard-Port | 502 |
| Slave-ID | 1 |
| FC Input lesen | 04 (Read Input Registers) |
| FC Holding lesen | 03 (Read Holding Registers) |
...
Die Module liegen jeweils 65 Adressen auseinander. Ein Raum belegt sieben
Adressen beginnend bei `base + 2`; konfiguriert werden können bis zu acht
Räume, wobei sechs die aktuelle Navigator-10-Vorgabe ist.

## Besondere Register

| Adresse | Beschreibung | Besondere Anmerkung |
|---------|-------------|-------------|
| 1999 | Fehlerquittierung | Darf NICHT dauerhaft beschrieben werden |
| 1696 | GLT-Heizanforderung | Muss zyklisch alle 10 Min. geschrieben werden |
| 1698 | GLT-Kühlanforderung | Muss zyklisch alle 10 Min. geschrieben werden |

## EEPROM-empfindliche Register

88 Register sind EEPROM-empfindlich und haben nur eine begrenzte Anzahl von Schreibzyklen. Die Integration warnt automatisch, wenn diese Register häufig geschrieben werden.

## Qualitätsregeln für Register

- Neue Register müssen aus dem Register-Schema der API stammen, nicht aus Ad-hoc-Code in Home Assistant.
- Modellspezifische Register müssen vor der Abfrage anhand der erkannten Modellfähigkeiten freigegeben werden.
- Sentinel-Werte werden kontextabhängig interpretiert; dokumentierte Enum-Werte haben Vorrang vor generischen Sentinels für „nicht verfügbar“.
- Abweichungen an realen Geräten werden als Kompatibilitätsnotizen und Regressionstests erfasst, bevor sie als unterstützte Funktionen angekündigt werden.
- Batches enthalten nur exakt angrenzende, sich nicht überlappende logische Bereiche und sind auf 40 Modbus-Wörter begrenzt.
- Ein individueller Roh-Schreibzugriff kann die Kodierung prüfen, aber keine sicheren Bereiche, kein EEPROM-Verhalten und keine Bedeutung für eine unbekannte Adresse ableiten.

## Register-Hoheit externer Energiemanager

Einige GLT/PV-Eingangsregister sind dafür vorgesehen, fortlaufend von einem
externen Energiemanager versorgt zu werden. Die offizielle iDM-Dokumentation
definiert beispielsweise Register 74 als den beschreibbaren aktuellen
PV-Überschuss und Register 4122 als die nur lesbare aktuelle
Leistungsaufnahme der Wärmepumpe. Wenn bereits ein Wechselrichter, ein E3DC,
ein Smartfox oder eine andere Steuerung einen GLT/PV-Wert schreibt, kann ein
Schreibzugriff aus Home Assistant ihn nur bis zum nächsten
Kommunikationszyklus dieser Steuerung überschreiben.

Vermeide mehrere Schreiber für dasselbe Register. Kläre, bevor du eine
Automation erstellst, welches System den Wert besitzt, und nutze die
Bibliotheks-Metadaten des Registers für Datentyp, Schreibklasse, Wertebereich
und Zyklusverhalten. Leite keine Semantik von einer Adresse ab, nur weil eine
andere Installation oder Firmware sie nutzt; diese Integration bildet
beispielsweise den Smart-Grid-Status und den konfigurierbaren variablen
Eingang als getrennte Register ab.

### Datentyp-Referenz für PV/Energiemanagement

Die Adressen im PV-Block haben nicht alle denselben Datentyp. Insbesondere der
Batterie-SOC darf nicht als Float über zwei Register kodiert werden.

| Adresse | Entität | Datentyp | Einheit | Anmerkungen |
|---------|--------|----------|------|-------|
| 74 | `pv_surplus` | FLOAT, wortgetauscht | kW | Beschreibbarer GLT-Eingang; kein EEPROM |
| 76 | `electric_heater_power` | FLOAT, wortgetauscht | kW | Beschreibbare GLT-Messung |
| 78 | `pv_production` | FLOAT, wortgetauscht | kW | Beschreibbare GLT-Messung |
| 82 | `house_consumption` | FLOAT, wortgetauscht | kW | Beschreibbare GLT-Messung |
| 84 | `battery_discharge` | FLOAT, wortgetauscht | kW | Beschreibbare GLT-Messung |
| 86 | `battery_soc` | INT16 mit Vorzeichen, ein Register | % | `0–100`; `-1` bedeutet nicht verfügbar |
| 88 | `pv_target_value` | FLOAT, wortgetauscht | kW | Modell-/Firmware-abhängig |

Beim Schreiben verwende immer die erzeugte Entität oder die Registerdefinition
aus der Bibliothek. Die erweiterte Aktion für Roh-Schreibzugriffe kann den
korrekten Datentyp nicht aus einer Adresse ableiten.
...
Von diesen Definitionen sind **451** beschreibbar, **88** EEPROM-empfindlich und **2** erfordern zyklische Schreibzugriffe. `FLOAT`-Werte belegen zwei Modbus-Wörter; die Tabelle zeigt dafür daher einen Adressbereich an. `R` bedeutet nur lesend, `RW` lesend/schreibend und `W` nur schreibend.

Die deutsche Beschreibung dient der Identifikation; der Registername in Codeform ist der verbindliche Schlüssel, den die Integration verwendet. Die Verfügbarkeit kann je nach Navigator-Modell und Firmware variieren.

### 74–999 · PV & Smart Grid (8)

| Adresse(n) | Beschreibung (DE) | Registername | Typ | Einheit | Zugriff | Anmerkung |
|-------------|------------------|---------------|------|------|--------|------|
...
### 1000–1199 · System & Wärmepumpe (64)
...
### 1350–1699 · Heizkreise & Anforderungen (123)
...
### 1700–1999 · Energie, Solar, GLT & Service (29)
...
| 1999 | Fehlerquittierung | `error_acknowledge` | UCHAR | — | W · nur schreibend | — |

### 2000–2999 · Zonenmodule (420)
...
### 4000+ · Navigator-10-Erweiterungen (25)
