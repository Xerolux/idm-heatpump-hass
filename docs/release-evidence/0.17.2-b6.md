# Release Candidate Evidence: 0.17.2-b6

Status: `BLOCKED` for stable promotion; beta publication is authorized.

This record captures the pre-publication state on 2026-09-18. Hardware
validation and the seven-day soak for this candidate remain pending.
See [the audit report](../dev/post-release-audit-2026-09-18.md) for the
regressions, local quality gates and read-only observation of the previous beta.

## Candidate

| Field | Value |
|---|---|
| Version / tag | `0.17.2-b6` / `v0.17.2-b6` |
| Commit SHA | |
| Published at (UTC) | |
| Artifact URL | https://github.com/Xerolux/idm-heatpump-hass/releases/download/v0.17.2-b6/idm_heatpump.zip |
| SHA-256 | |
| API requirement | |
...
Local preflight: 1781 tests passed, 2 skipped; 95.27% integration coverage
and 100% config-flow coverage. Strict mypy, Ruff and the production transport
import/disconnect smoke test passed. Runtime dependencies remain unchanged.
