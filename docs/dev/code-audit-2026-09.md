...
| E1 | done | `tests_ha/` boots a genuine Home Assistant (temp config dir, the repository's `custom_components`, a spec'd fake Modbus client answering like a Navigator 10) and walks one config entry through setup, reload and unload with an entity-count, state and no-leaked-tasks assertion. CI runs it as the `smoke` job in `ci.yml` on every push and pull request; locally through the `test_ha/` venv. The stubbed suite in `tests/` stays the fast path. |
