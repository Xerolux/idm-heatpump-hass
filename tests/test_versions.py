...
def test_runtime_versions_exposes_integration_and_dependencies() -> None:
    with patch(
        ...
        versions = runtime_versions("0.8.1-beta.22")

    assert versions.integration == "0.8.1-beta.22"
...
async def test_async_runtime_versions_runs_sync_lookup_in_thread() -> None:
    ...
    with (
        ...
        result = await async_runtime_versions("0.8.1-beta.22")

    assert result is expected
    to_thread.assert_awaited_once_with(lookup, "0.8.1-beta.22")
