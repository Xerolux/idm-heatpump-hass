...
from custom_components.heatpump_idm import (
    IdmHeatpumpData,
    async_setup,
    async_setup_entry,
    async_unload_entry,
...
class TestAsyncSetup:
    async def test_calls_setup_services(self, mock_hass):
        with patch(
            "custom_components.heatpump_idm.services.async_setup_services",
            new_callable=lambda: lambda: AsyncMock(return_value=None),
        ) as mock_setup:
            with patch(
                "custom_components.heatpump_idm.async_setup.__wrapped__",
                create=True,
            ):
                pass
...
class TestAsyncSetupEntry:
    ...
    async def test_sets_runtime_data(self, mock_hass):
        ...
        with patch(
            "custom_components.heatpump_idm.get_idm_client",
            return_value=mock_client,
        ), patch(
            "custom_components.heatpump_idm.IdmCoordinator",
            return_value=mock_coordinator,
        ), patch(
            "custom_components.heatpump_idm.async_get_integration",
            return_value=MagicMock(manifest={"version": "0.5.0"}),
        ), patch(
            "custom_components.heatpump_idm.get_all_sensor_descriptions",
            return_value=[],
        ), patch(
            "custom_components.heatpump_idm.get_all_binary_sensor_descriptions",
            return_value=[],
        ), patch(
            "custom_components.heatpump_idm.get_all_number_descriptions",
            return_value=[],
        ), patch(
            "custom_components.heatpump_idm.get_all_select_descriptions",
            return_value=[],
        ), patch(
            "custom_components.heatpump_idm.get_all_switch_descriptions",
            return_value=[],
        ):
    ...
    async def test_forwards_entry_setups(self, mock_hass):
        ...
        with patch(
            ...
            "custom_components.heatpump_idm.get_all_sensor_descriptions", return_value=[]
        ), patch(
            "custom_components.heatpump_idm.get_all_binary_sensor_descriptions", return_value=[]
        ), patch(
            "custom_components.heatpump_idm.get_all_number_descriptions", return_value=[]
        ), patch(
            "custom_components.heatpump_idm.get_all_select_descriptions", return_value=[]
        ), patch(
            "custom_components.heatpump_idm.get_all_switch_descriptions", return_value=[]
        ):
            await async_setup_entry(mock_hass, entry)
...
class TestAsyncUnloadEntry:
    async def test_unloads_platforms(self, mock_hass):
        ...
        with patch(
            "custom_components.heatpump_idm.services.async_unload_services",
            AsyncMock(),
        ):
...
class TestAsyncSetupEntryOptions:
    ...
    def _common_patches(self, mock_client, mock_coordinator):
        return [
            patch("custom_components.heatpump_idm.get_idm_client", return_value=mock_client),
            patch("custom_components.heatpump_idm.IdmCoordinator", return_value=mock_coordinator),
            patch("custom_components.heatpump_idm.async_get_integration",
                  return_value=MagicMock(manifest={"version": "0.5.0"})),
            patch("custom_components.heatpump_idm.get_all_sensor_descriptions", return_value=[]),
            patch("custom_components.heatpump_idm.get_all_binary_sensor_descriptions", return_value=[]),
            patch("custom_components.heatpump_idm.get_all_number_descriptions", return_value=[]),
            patch("custom_components.heatpump_idm.get_all_select_descriptions", return_value=[]),
            patch("custom_components.heatpump_idm.get_all_switch_descriptions", return_value=[]),
        ]
    ...
    async def test_default_port_used_when_missing(self, mock_hass):
        ...
        patches[0] = patch("custom_components.heatpump_idm.get_idm_client", side_effect=_capture_client)
    ...
    async def test_hide_unused_passed_to_coordinator(self, mock_hass):
        ...
        patches = self._common_patches(mock_client, mock_coordinator)
        patches[1] = patch("custom_components.heatpump_idm.IdmCoordinator",
