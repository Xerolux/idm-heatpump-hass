"""Tests for the idm-heatpump-api logging noise filter.

The pymodbus filter went away with the dependency: idm-heatpump-api 2.0.0 no
longer imports pymodbus, so there is no ``pymodbus.logging`` logger to quieten.
"""
...
class TestInstallPymodbusLogFilter:
    def setup_method(self) -> None:
        ...
        for logger_name in (LIBRARY_LOGGER_NAME,):
